...
function NavBar(props) {
    ...
    var [showG, setShowG] = React.useState();
        if(showH==true){
            showG=showS=showV=!showH; 
        }
        if(showG==true){
            showH=showS=showV=!showG; 
        }
        if(showS==true){
            showG=showH=showV=!showS; 
        }
        if(showV==true){
            showG=showS=showH=!showV; }
...
<div  onClick={() => setShowH(!showH)} className={`${props.home} ${showH ? "border-b-4 border-red-500 duration-300" : ""}`}>
...
<div  onClick={() => setShowV(!showV)} className={`${props.video} ${showV ? "border-b-4 border-red-500 duration-300" : ""}`}>
...
<div  onClick={() => setShowS(!showS)} className={` ${props.shop}${showS ? "border-b-4 border-red-500 duration-300" : ""}`}>
...
<div  onClick={() => setShowG(!showG)} className={` ${props.group}${showG ? "border-b-4 border-red-500 duration-300" : ""}`}>
...
<svg className="h-6 pt-1 w-6 mx-auto" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path fill="none" d="M0 0h24v24H0z"/><path d="M9.55 11.5a2.25 2.25 0 1 1 0-4.5 2.25 2.25 0 0 1 0 4.5zm.45 8.248V16.4c0-.488.144-.937.404-1.338a6.473 6.473 0 0 0-5.033 1.417A8.012 8.012 0 0 0 10 19.749zM4.453 14.66A8.462 8.462 0 0 1 9.5 13c1.043 0 2.043.188 2.967.532.878-.343 1.925-.532 3.033-.532 1.66 0 3.185.424 4.206 1.156a8 8 0 1 0-15.253.504zm14.426 1.426C18.486 15.553 17.171 15 15.5 15c-2.006 0-3.5.797-3.5 1.4V20a7.996 7.996 0 0 0 6.88-3.914zM12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm3.5-9.5a2 2 0 1 1 0-4 2 2 0 0 1 0 4z"/></svg></div></div>
           ...
           <div className=" pb-1 ">
               <button style={{color:"#ed0e3f"}} className=" active:ring-offset-4 active:ring ring-pink-800 py-2 px-3 text-sm md:text-lg duration-200 rounded-md font-medium  hover:bg-gray-100">تسجيل الدخول</button>
